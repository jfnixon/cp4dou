...
openshift_version=4.8.13  # 20211019, fnixon
...
if [[ $STORAGEOPTION == "portworx" ]]; then
  runuser -l $SUDOUSER -c "wget $ARTIFACTSLOCATION/artifacts/px-install.yaml$ARTIFACTSTOKEN -O $INSTALLERHOME/openshiftfourx/px-install.yaml"
  runuser -l $SUDOUSER -c "wget $ARTIFACTSLOCATION/artifacts/px-storageclasses.yaml$ARTIFACTSTOKEN -O $INSTALLERHOME/openshiftfourx/px-storageclasses.yaml"
...
if [[ $STORAGEOPTION == "nfs" ]]; then
  ...
  runuser -l $SUDOUSER -c "wget $ARTIFACTSLOCATION/artifacts/nfs-template.yaml$ARTIFACTSTOKEN -O  $INSTALLERHOME/openshiftfourx/nfs-template.yaml"
